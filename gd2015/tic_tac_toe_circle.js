...
var opacity_scale = d3.scale.linear().range([0.2, 0.4]);
...
d3.text("./tictactoe.txt", function(data) {
  ...
  function find_important_paths(node, condition) {
    ...
    important_child.red_important = true;

    find_important_paths(important_child, condition);
  }
  for (var i = 0; i < N; i++) {
    root.to[i].red_important = true;
    find_important_paths(root.to[i], "o_score");
  }
  root.red_important = true;
  ...
  _.forEach(vertices, function(vertex) {
    g.setNode(vertex.configuration, {
      ...
      red_important: vertex.red_important,
      win: vertex.win
    });
  });
 ...
 _.forIn(g._edgeObjs, function(v, k) {
   ...
   edges_data.push({
     ...
     red_important: source.red_important && target.red_important
   });
 ...
 _.find(edges_data, function(edge) {
   return edge.source.label == '------X-O' && edge.target.label == '--XX----O';
 }).red_important = false;
 //_.find(edges_data, function(edge) {
  ...
  var link = svg.selectAll("path.link")
      ...
      .style("stroke-opacity", function(d) {
        if (d.red_important) {
          ...
          if (d.o_score > 1800) {
            return 0.5;
          } else if (d.o_score > 1000) {
            return 0.2;
          }
          if (d.x_score > 1 && d.o_score < 400) {
            return 0.05;
          } else if (!d.o_score || d.o_score < 2){
            return 0.5;
          } else {
            return opacity_scale(-d.o_score);
          }
        }
      });
